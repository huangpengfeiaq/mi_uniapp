	...
	<view>
		...
		<view class="popup spec" :class="specClass" @touchmove.stop.prevent="discard" @tap="hideSpec">
			...
			<view class="layer" @tap.stop="discard">
				<view class="content">
					...
					<view class="sp">
						<view v-for="(item,index) in productStyleList" :class="[index==selectSpec?'on':'']" @tap="setSelectSpec(item,index)"
						 :key="index">{{item.styleVersion}}</view>
					</view>
					<view class="title">颜色</view>
					<view class="sp">
						<view v-for="(item,index2) in productStyleList" :key="index2">{{item.styleColor}}</view>
					</view>
		...
		<view class="swiper-box">
			<swiper circular="true" autoplay="true" @change="swiperChange">
				<swiper-item v-for="swiper in productStyleList" :key="swiper.styleId">
					<image :src="swiper.styleCover" @tap="toSwiper(swiper)"></image>
				</swiper-item>
			</swiper>
			<view class="indicator">{{currentSwiper+1}}/{{productStyleList.length}}</view>
		</view>
		<!-- 标题 价格 -->
		<view class="info-box goods-info">
			<view class="productName">{{product.productName}}</view>
			<view>
				<label class="productPromotion" v-if="product.productPromotion">[{{product.productPromotion}}]</label>
				<label class="productIntroduction">{{product.productIntroduction}}</label></view>
			<view class="productPrice" v-show="stylePromotionalPrice">￥{{stylePromotionalPrice}}<label class="del">￥{{stylePrice}}</label></view>
			<view class="productPrice" v-show="!stylePromotionalPrice">￥{{stylePrice}}</view>
		</view>
		<!-- 服务-规则选择 -->
		...
		<view class="description">
			...
			<view class="content">
				<rich-text :nodes="product.productArticleContent"></rich-text>
			</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		onLoad(option) {
			...
			uni.request({
				...
				method: 'GET',
				success: (res) => {
					this.productStyleList = res.data.content;
					this.setSelectSpec(res.data.content[0], 0);
				}
			})
		},
		onReady() {
